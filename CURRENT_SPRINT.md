...
- **Front auth** : `app/(auth)/login.tsx` livré ; `register.tsx` est un placeholder
  (écran réel = TLX-026). Infra session déjà en place (token-store trousseau,
  SessionProvider, intercepteur refresh single-flight de TLX-009).
