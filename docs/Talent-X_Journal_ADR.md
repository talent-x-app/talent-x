...
| ADR-59 | **Notification de la discussion de séance** (`session_comment`, TLX-268) : le fil de TLX-118 était livré sans canal d'alerte — ni la question de l'athlète ni la réponse du coach ne prévenaient personne. CHECK étendu (additif, patron ADR-46/50), fan-out selon la nature partagée du fil, `resourceId` navigable **par son destinataire** (leçon TLX-266) | Accepté |
...
Décision complète : [`docs/adr/ADR-59-notification-discussion-de-seance.md`](adr/ADR-59-notification-discussion-de-seance.md).

**Statut : Accepté** (2026-08-21, périmètre arbitré par le propriétaire). L'écran athlète invite « Pose une question à ton coach » ; l'athlète la pose et **le coach n'en est jamais informé**. Le coach répond, **l'athlète n'en est jamais informé non plus**. Mesuré sur appareil (QA-04.7) : **0 notification dans les deux sens** — un zéro qui est une mesure, la même sonde comptant 13 notifications ce jour-là. Cause : `comments.service.ts` n'a qu'un chemin de notification, gardé par `target === 'performance'`, et la taxonomie ADR-22 **n'a aucun type** pour un commentaire de séance — le fil (TLX-118) a été livré sans son canal d'alerte, et aucune ligne de code ne pouvait le signaler puisque le type manquait à la racine. **D1** la fonctionnalité reste au MVP avec un type dédié `session_comment` (CHECK étendu par migration additive, patron ADR-46 puis ADR-50) ; la retirer était l'alternative, écartée — le fil est complet et correct, il ne lui manquait que d'être annoncé. **D2** fan-out suivant la nature **partagée** du fil : question d'athlète → **le coach propriétaire** seul ; réponse du coach → **tous les athlètes affectés** (ADR-30), qui la voient tous. Deux exclusions : l'auteur ne se notifie jamais lui-même, et un athlète qui écrit ne réveille pas les autres athlètes — sans quoi un fil à dix affectés devient une chambre d'écho. **D3** le `resourceId` est **celui que son destinataire sait ouvrir** : `sessionId` pour le coach, **son** `assignmentId` pour chaque athlète. Les deux routes s'appellent `session/[id]` et ne prennent pas la même chose — exactement le piège de TLX-266, résolu en émettant une notification par destinataire plutôt qu'un identifiant unique partagé. **D4** garde `performanceFeedback` réutilisée plutôt qu'une colonne dédiée (patron ADR-46) : c'est la même conversation coach ↔ athlète, une étape plus tôt ; limite assumée et écrite — couper le feedback coupe aussi les questions de séance. **D5** push générique (ADR-10), `actorId` pour le feed nominatif (ADR-55), `dedupeKey` portant **le destinataire**, sans quoi le fan-out se dédupliquerait contre lui-même et un seul athlète serait servi. **Cas rattrapé au passage** : le fil de séance disparaissait dès qu'une perf était soumise — or c'est la séquence normale (question avant, perf pendant, réponse après) ; notifier vers un écran où le fil n'est plus rendu aurait été un nouveau TLX-266, les deux fils coexistent donc. **Coût** : fan-out en O(affectés) — trente jobs pour une réponse sur une séance de groupe à trente, acceptable au MVP, à surveiller. Écartés : réutiliser `performance_feedback` (rendrait faux d'un coup un libellé, une clé de cache et un commentaire), ne notifier que l'auteur de la question, une colonne de préférence dédiée, différer la fonctionnalité.
