/* ============================================================
   Talent-X Design System — tokens (React Native / Expo)
   Valeurs typées, INDÉPENDANTES de react/react-native (testables en Node).
   Source de vérité : ./tokens.json (format W3C). Ce fichier en est le
   portage RN fidèle ; un test de parité (tokens.test.ts) garantit l'absence
   de dérive. Aucune valeur en dur dans les composants : tout passe par useTheme().
   ============================================================ */

/* ---------- Rampes primitives (indépendantes du thème) ---------- */
...
/* Gradient signature X — à passer à expo-linear-gradient.
   Réservé à la marque / un unique accent héros. Jamais en fond d'UI ni sur du texte courant. */
...
/* ---------- Couleurs sémantiques par thème ---------- */
...
/* ---------- Échelles indépendantes du thème ---------- */
...
/* Élévation sombre = bordure fine (+ glow accent), pas d'ombre portée. Élévation claire = ombres douces. */
...
export const darkTheme: Theme = {
  ...
  touchTarget,
  motion,
  elevation: elevation.dark as unknown as typeof elevation.light,
  opacity,
};
