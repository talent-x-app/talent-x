# trajet-sncf — veille TGV Max Jeune à 0 € (Paris ↔ Nancy)
...
notification push sur le téléphone via [ntfy](https://ntfy.sh).
...
3. **Créer le secret GitHub** : sur github.com → ce repo → **Settings** →
   **Secrets and variables** → **Actions** → **New repository secret** :
   - Name : `NTFY_TOPIC`
   - Secret : le nom du topic choisi en 2.
4. **Tester** : onglet **Actions** → workflow **maxwatch** → **Run workflow**
   ...
   groupée.
5. C'est tout. Le cron prend le relais (toutes les 15 à 30 min en pratique,
...
doit être EXACTEMENT celle de l'Open Data (ex. `PARIS (intramuros)`, `NANCY`) —
la liste complète s'obtient en lançant le workflow **gares** dans l'onglet
Actions.
...
- `.github/workflows/gares.yml` — outil manuel : liste des gares + dry-run.
